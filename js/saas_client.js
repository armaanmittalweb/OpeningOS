...
(function (global) {
  ...
  async function request(method, path, body, opts) {
    opts = opts || {}; const cfg = config(); if (!cfg.backendUrl) throw new Error('Connection URL is not configured.');
  ...
  function panel() {
    ...
    const cfg = config(); const url = field('Connection URL', cfg.backendUrl || ''); const email = field('Email', cfg.user && cfg.user.email || '', 'email'); const pass = field('Password', '', 'password'); const display = field('Display name', cfg.user && cfg.user.displayName || ''); const invite = field('Coach invite token', ''); const username = field('Import username', '');
    ...
    main.appendChild(h('section', { class: 'settings-group', id: 'settings-saas' }, [
      h('div', { class: 'settings-group-head' }, [h('h3', {}, ['Cloud account and collaboration']), h('p', { class: 'muted' }, ['Account, sync, coach sharing, game imports and recovery controls.'])]),
      h('div', { class: 'setting-row' }, [h('div', {}, [h('div', { class: 'label' }, ['Cloud connection']), h('div', { class: 'desc' }, ['Use the configured OpeningOS Cloud connection.'])]), h('div', { class: 'stack-sm' }, [url, h('button', { class: 'btn btn-sm', on: { click: () => { api.configure({ backendUrl: url.querySelector('input').value }); toast('Cloud connection saved', 'good'); } } }, ['Save'])])]),
      h('div', { class: 'setting-row' }, [h('div', {}, [h('div', { class: 'label' }, ['Account & recovery']), h('div', { class: 'desc' }, ['Sign in, reset your password and manage recovery options.'])]), h('div', { class: 'stack-sm' }, [display, email, pass, h('div', { class: 'row', style: { gap: '8px', flexWrap: 'wrap' } }, [h('button', { class: 'btn btn-sm', on: { click: () => run(api.signup(email.querySelector('input').value, pass.querySelector('input').value, display.querySelector('input').value), 'Signed up') } }, ['Sign up']), h('button', { class: 'btn btn-sm', on: { click: () => run(api.login(email.querySelector('input').value, pass.querySelector('input').value), 'Signed in') } }, ['Login']), h('button', { class: 'btn btn-sm', on: { click: () => run(api.requestPasswordReset(email.querySelector('input').value), 'Reset email queued') } }, ['Reset password']), h('button', { class: 'btn btn-sm', on: { click: () => api.regenerateRecoveryCodes().then(r => alert('Recovery codes:\n' + (r.codes || []).join('\n'))).catch(e => toast(e.message, 'bad')) } }, ['Recovery codes'])])])]),
      h('div', { class: 'setting-row' }, [h('div', {}, [h('div', { class: 'label' }, ['Cloud sync']), h('div', { class: 'desc' }, ['Keep your repertoire safe across devices.'])]), h('div', { class: 'row', style: { gap: '8px', flexWrap: 'wrap', justifyContent: 'flex-end' } }, [h('button', { class: 'btn btn-sm', on: { click: () => run(api.pushSnapshot(), 'Cloud saved') } }, ['Save to cloud']), h('button', { class: 'btn btn-sm', on: { click: () => run(api.pullSnapshot(), 'Cloud restored') } }, ['Restore from cloud']), h('button', { class: 'btn btn-sm', on: { click: () => run(api.graphPush(), 'Workspace saved') } }, ['Save repertoire map']), h('button', { class: 'btn btn-sm', on: { click: () => run(api.graphPull(), 'Workspace restored') } }, ['Restore repertoire map'])])]),
      ...
      h('div', { class: 'setting-row' }, [h('div', {}, [h('div', { class: 'label' }, ['Game imports and plan']), h('div', { class: 'desc' }, ['Import games, review openings and manage your plan.'])]), h('div', { class: 'stack-sm' }, [username, h('div', { class: 'row', style: { gap: '8px', flexWrap: 'wrap' } }, [h('button', { class: 'btn btn-sm', on: { click: () => run(api.createImportJob('lichess', { username: username.querySelector('input').value }), 'Lichess import queued') } }, ['Import Lichess']), h('button', { class: 'btn btn-sm', on: { click: () => run(api.createImportJob('chesscom', { username: username.querySelector('input').value }), 'Chess.com import queued') } }, ['Import Chess.com']), h('button', { class: 'btn btn-sm', on: { click: () => api.checkout('pro').then(r => r.url ? location.href = r.url : alert(JSON.stringify(r, null, 2))).catch(e => toast(e.message, 'bad')) } }, ['Upgrade']), h('button', { class: 'btn btn-sm', on: { click: () => api.adminOverview().then(r => alert(JSON.stringify(r, null, 2))).catch(e => toast(e.message, 'bad')) } }, ['Admin'])])])])
    ]));
    const side = document.querySelector('.settings-side'); if (side && !side.querySelector('[href="#settings-saas"]')) side.appendChild(h('a', { href: '#settings-saas', on: { click: e => { e.preventDefault(); document.getElementById('settings-saas').scrollIntoView({ behavior: 'smooth', block: 'start' }); } } }, ['Cloud']));
  }
